  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || "Failed to add sales data");
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 p-6">
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-xl p-6">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          {/* Fields */}
          {["Heading", "SubHeading", "Description", "Type", "Budget"].map(
            (field, idx) => {
              const valueMap = {
                Heading: heading,
          ...
          {files.length > 0 && (
            <div className="flex flex-wrap gap-3 mt-4">
              {files.map((file, idx) => (
                <div
                  key={idx}
                  className="flex flex-col items-center gap-2 bg-indigo-100 text-indigo-700 rounded-lg px-2 py-1 text-xs w-20"
                >
        ...
        {showConfirm && (
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl shadow-lg p-6 max-w-md w-full">
              ...
              {subHeading && (
                <p className="mb-2">
                  <strong>SubHeading:</strong> {subHeading}
                </p>
              )}
              <p className="mb-2">
